...
import { status, StatusSummary } from "../../helpers/git.js";
import { findRoot, listLibs, listModules } from "../../helpers/workspace.js";
...
export default class Status extends Command {
  ...
  async _view(category: string, workspace: string, options: StatusOptions) {
    const task = async (m: string) => await status(m, !options.offline);
    switch (category) {
      case "root":
  ...
  _remoteStatusSymbol(ahead: number, behind: number) {
    if (ahead && behind) {
      return chalk.bold("±");
    }
    if (ahead && !behind) {
  ...
  _statusMsg(category: string, info: StatusSummary, indent: string) {
    const branch = (info: StatusSummary) => {
      const b = info.status.current ?? "";
      if (b.length > 16) {
        return (b.substring(0, 16) + "…").padEnd(18);
      }
