...
export default class WorkspaceLoad extends Command {
  ...
  public async run(): Promise<void> {
    ...
    const {flags} = await this.parse(WorkspaceLoad)
    ...
    const task = async (repo: { dir: string; options: CheckoutOptions }) =>
      checkout(repo.dir, repo.options)
  ...
  msg(category: string, result: CheckoutResult): string {
    const ref = (status: StatusResult): string => `${status.current}@${status.ref.slice(0, 8)}`
